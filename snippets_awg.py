...
def get_dynamic_charge_state_init(length_orange_mus=800):
...
class SSR(object):
    ...
    def __init__(self, mcas, frequencies, wait_dur=1., **kwargs):
        ...
        self.set_dur_step()
        self.set_dynamic_control(kwargs)
        self.kwargs = kwargs
    ...
    def set_dynamic_control(self, kwargs):
        self.dynamic_control = kwargs['dynamic_control'] if 'dynamic_control' in kwargs else False
        if type(self.dynamic_control) != bool:
            raise Exception("The variable dynamic_control has to be a boolean value")
