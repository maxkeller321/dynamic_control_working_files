...
class NuclearOPs(DataGeneration):
    ...
    def setup_rf(self, current_iterator_df):
        multiple_mcas = self.ret_mcas(current_iterator_df)
        if type(multiple_mcas) == list:
            self.mcas = multiple_mcas[0]
            for item in multiple_mcas:
                if self.dynamic_control:
                    item.dynamic_control = True
                pi3d.mcas_dict[item.name] = item
        else:
            self.mcas = multiple_mcas
