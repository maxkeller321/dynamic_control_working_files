...
class MultiChSeq:
    def __init__(self, ch_dict=None, **kwargs):
        ...
        self.init_sequences()
        self.status = 0
        self.sync_master_and_slave = True
    ...
    def finalize(self, ignore_max_avg_power=False, notify=True):
        ...
        if settings.master_awg is not None and settings.master_awg in self.ch_dict and len(self.ch_dict) > 1 and self.sync_master_and_slave:
    ...
    def set_all_advancement_modes_to_AUTO(self):
        """
        This function is just for the dynamic control mode. Here the only advancement-mode between segments & sequences is AUTO
        """
        for awg_str in self.ch_dict.keys():
            for ch in self.ch_dict[awg_str]:
                try: 
                    self.sequences[awg_str][ch].advance_mode = "AUTO"
                except: 
                    pass 

                for i in range(len(self.sequences[awg_str][ch].data_list)):
                    self.sequences[awg_str][ch].data_list[i].advance_mode = "AUTO"
    
    def set_sequence_advancement_modes_to_COND(self): 
        """
        This function is just for the switching from the dynamic control mode to the normal mode
        """
        for awg_str in self.ch_dict.keys():
            for ch in self.ch_dict[awg_str]:
                try: 
                    self.sequences[awg_str][ch].advance_mode = "COND"
                except: 
                    pass
    ...
    def write_to_awg_memory(self, ignore_max_avg_power=False, abort=None, notify=True):
        if self.dynamic_control:
            self.set_all_advancement_modes_to_AUTO()
    ...
    def initialize(self, trigger_mode='triggered'):
        for awg_str, chl in settings.ch_dict_full.items():
            for ch in chl:
                ...
                self.mcas_dict.awgs[awg_str].ch[ch].initialize_sequence(sequence, trigger_mode=trigger_mode)
