...
class DataList(collections.MutableSequence):
    ...
    def missing_smpl_step(self):
        if len(self.list) > 0: # Add the sync/ sample markers also to the _missing_sample_ wavestep
            if self.list[-1].smpl_marker:
                return WaveStep(length_smpl=self.missing_smpl, smpl_marker=True, name='_missing_smpls_')
            elif self.list[-1].sync_marker:
                return WaveStep(length_smpl=self.missing_smpl, sync_marker=True, name='_missing_smpls_')
            else:
