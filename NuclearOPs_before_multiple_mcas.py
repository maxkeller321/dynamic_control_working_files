...
class NuclearOPs(DataGeneration):
    ...
    def __init__(self):
        ...
        self.dynamic_charge_init = False 
        self.dynamic_sequence_switching = False
    ...
    def prepare_for_dynamic_mode(self):
        print("preparing for dynamic mode")
        pi3d.md.stop_awgs()
        for key in pi3d.md:
            if key == "green":
                green = pi3d.md['green']
    ...
    def abort_function(self):
        ...
        if self.dynamic_charge_init or self.dynamic_sequence_switching:
            pi3d.md.stop_awgs()
            for key in pi3d.md:
                if key == "green":
                    green = pi3d.md['green']
            ...
            for idx, _ in enumerate(self.iterator()):
                ...
                while True:
                    ...
                    if self.dynamic_charge_init:
                        ...
                        dyn_control = DynamicControl(normal_dyn_mode=False)# maybe make out of dynamic control just a sequence for ...
                        mcas_charg_init = dyn_control.create_charge_state_init_sequence()
                        pi3d.mcas_dict[mcas_charg_init.name] = mcas_charg_init
                        pi3d.mcas_dict[self.mcas.name] = self.mcas
                        print("charge_init_was_written")
                        self.mcas = mcas_charg_init
                        trigger_mode = 'triggered'

                        pi3d.md['wait'] = wait
                        pi3d.md['green'] = green

                    elif self.dynamic_sequence_switching:
